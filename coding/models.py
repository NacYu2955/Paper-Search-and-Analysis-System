...
class Agent:
    ...
    def infer_score(self, prompts):
        try:
            #print(f"开始处理 {len(prompts)} 个prompt")
            ...
            for i in range(0, len(prompts), batch_size):
                ...
                #print(f"子批次形状: {batch_input_ids.shape}")
            ...
            #print(f"所有批次处理完成，总概率数量: {len(all_probs)}")
            return all_probs
            
        except Exception as e:
            #print(f"infer_score方法出错: {str(e)}")
            #print(f"错误类型: {type(e).__name__}")
            import traceback
            #print(f"错误堆栈: {traceback.format_exc()}")
            return [0.0] * len(prompts) if prompts else []
